...
EMPLACEMENT_SAUVEGARDE = "sauvegardes/" #for ListLink only
...
class ListeDeTaches(NinjatodolaObject):
    ...
    def action_objet(self, choix_retour):
        ...
        if choix_retour == "$": # ajouter un objet dans la LDT
            ...
            choix_objets_dispo = ["A", "D", "L"]
            choix_objets_dispo_str = ["Quel type d'objet?: ", "- Application (A)", "- Dossier (D)", "- Link (L)"]
            ...
            if type_objet == "L": #Create a link
                position_link = menu(message_position_tache, type_input="nombre") #demander la position
                objet = ListLink(self.rang_indentation + 1, position_link, self)
                self.ajout_objet(objet, position_link)
